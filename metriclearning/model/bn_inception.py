...
model_urls = {
    ...
    # FIXME: add hash suffix to the filename to be able to download it from internet
    'bn_inception': 'https://github.com/dichotomies/proxy-nca/raw/master/net/bn_inception_weights_pt04.pt',
}

transform_parameters = {
    'rgb_to_bgr': True, # imported from caffee, therefore BGR needed
    'intensity_scale': [[0, 1], [0, 255]], # expects values in [0, 255]
    'mean': [104, 117, 128],
    'std': [1, 1, 1]
}

def bn_inception(pretrained=False, **kwargs):
    r"""BNInception model architecture.
    Args:
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.utils.model_zoo.load_url(model_urls['bn_inception'])
        )
        return model
