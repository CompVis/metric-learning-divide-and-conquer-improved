...
def train_kmeans(x, num_clusters=1000, gpu_ids=None, niter=100, nredo=1, verbose=0, seed=1234):
    """
    Runs k-means clustering on one or several GPUs

    seed: random seed for clsutering.
          If None will use the default value from faiss.Clustering
            constructor (always fixed to some default constant)
    """
    ...
    #imbalance = faiss.vector_float_to_array(kmeans.imbalance_factor)
    logging.debug(f" # Final clustering objective={objective[-1]:.2f}")
...
def do_clustering(features, num_clusters, gpu_ids=None,
    ...
    logging.debug(f'FAISS: clustering [niter={niter}, nredo={nredo}]...')
    t0 = time.time()
    centroids = train_kmeans(features, num_clusters, gpu_ids=gpu_ids,
                             niter=niter,
                             nredo=nredo,
                             verbose=verbose,
                             seed=seed)
...
def find_nearest_neighbors(x, queries=None, k=5, gpu_id=None):
    ...
    distances (ndarray): Distances to the nearest neighbors (Squared Euclidean distances). [M x k]
